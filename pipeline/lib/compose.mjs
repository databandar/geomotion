...
export async function compose(script, timings) {
  ...
  if (tour) {
    layers.push({
      ...
      cameraOvershoot: script.cameraOvershoot !== false,
      cameraBow: script.cameraBow ?? 0.35,
      borderCasing: true,
      dimOthers: 0.45,
      intro: Math.max(0.5, introLength),
      introTrace: coldOpen ? false : true,
      // The outro covers every beat after the tour, so the camera stays pulled
      // back through the end card as well as the labelled map.
