...
describe('static tracks', () => {
  ...
  it('carries a non-numeric value untouched', () => {
    expect(evalTrack(staticTrack('slideUp'), 5, holdValue)).toBe('slideUp');
  });
});
...
describe('keyframed tracks', () => {
  ...
  it('falls back rather than throwing on a track with no keys', () => {
    ...
    expect(evalTrack(keyframedTrack<number>([]), 3, lerpNumber, -1)).toBe(-1);
  });
...
describe('interpolation is the caller\'s choice', () => {
  it('blends a non-numeric channel however it was told to', () => {
    ...
    expect(evalTrack(t, 0.5, shortWay)).toBeCloseTo(360, 5);
  ...
  it('holds a value that cannot be blended', () => {
    ...
    expect(evalTrack(t as never, 1, holdValue)).toBe('a');
    expect(evalTrack(t as never, 2, holdValue)).toBe('b');
  });
});

describe('the kinds M1 does not evaluate', () => {
  it('reports bound and expr as unsupported instead of pretending', () => {
    /*
     * Declared in the union from the start so every switch over a track is exhaustive
     * now — adding their evaluation later is then a compile error at each site that must
     * handle them, rather than a silent fallthrough returning a default.
     */
    expect(trackKindSupported({ kind: 'bound', ref: 'geo:in-wb', path: 'population' })).toBe(false);
  ...
  it('returns the fallback rather than a wrong number', () => {
    expect(evalTrack({ kind: 'bound', ref: 'x', path: 'y' }, 1, lerpNumber, 99)).toBe(99);
  });
});
