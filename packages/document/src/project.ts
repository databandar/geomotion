...
/**
 * A fresh layer of `type`, with `opts` written over its defaults.
 *
 * The defaults are the node type's own (schema/layers.ts) rather than a switch here: one
 * place says what a `regions` layer is, and construction, the load repair and the inspector
 * all read it. `opts` is typed as `Partial<Layer>` across the union, which is looser than any
 * one type — the cast is v1's, kept because the call sites pass literal objects the compiler
 * checks against the type they then assert.
 */
export function createLayer(type: LayerType, at: number, opts: Partial<Layer> = {}): Layer {
  const def = LAYER_TYPES[type];
  return { ...(def.create(at) as Layer), ...(opts as object) } as Layer;
}
...
function fillLayer(raw: Record<string, unknown>): Layer {
  ...
  /*
   * A type this build does not know keeps its own identity and its own fields, and is filled
   * only with what every layer has. That is a document from a *newer* build — or one a
   * plugin's node type made — and rewriting it into something this build understands would
   * destroy it on the next save. Nothing draws it (the evaluator's switch matches no branch)
   * and the panel lists it, which is the honest reading of "we do not know what this is".
   */
  const def = nodeTypeDef(String(raw.type));
  const defaults = (def ? def.create(at) : layerBase(at)) as Layer;
