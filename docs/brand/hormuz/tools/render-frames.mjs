/**
 * Hormuz renderer. Drives the studio's headless API and writes PNGs.
 *
 *   node tools/render-frames.mjs check ...   -> spot frames into ../check
 *   node tools/render-frames.mjs all         -> every frame at project fps into ../frames
 *
 * Needs `pnpm dev` running (localhost:5173).
 */
...
const project = JSON.parse(readFileSync(`${S}/project.geomotion.json`, 'utf8'));
